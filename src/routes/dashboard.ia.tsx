    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Recomendações, previsões e ranking inteligente.
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <Card className="p-6 lg:col-span-2 relative overflow-hidden">
          ...
          <Badge className="gradient-accent text-accent-foreground border-0 mb-3">
            <Sparkles className="h-3 w-3 mr-1" /> IA Insight
          </Badge>
          <h2 className="text-xl font-semibold mb-2">
            3 cursos com risco crescente este semestre.
          </h2>
          <p className="text-sm text-muted-foreground mb-6">
            O modelo detectou queda de frequência consistente em Direito (2º ano), Contabilidade
            (3º) e Marketing (1º). Recomenda-se intervenção pedagógica em até 2 semanas.
          </p>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={s.n} className="flex items-center gap-3">
                <div
                  className={`h-7 w-7 rounded-md text-xs font-bold flex items-center justify-center ${i === 0 ? "gradient-accent text-accent-foreground" : "bg-muted"}`}
                >
                  #{i + 1}
                </div>
        ...
        {[
          {
            i: AlertTriangle,
            t: "Alerta de risco",
            d: "14 estudantes apresentam queda crítica de frequência.",
            c: "destructive",
          },
          {
            i: TrendingUp,
            t: "Previsão positiva",
            d: "Engenharia Informática deve subir 4% nas notas finais.",
            c: "primary",
          },
        ].map((x) => (
          <Card key={x.t} className="p-5 flex gap-4 items-start">
            <div
              className={`h-10 w-10 rounded-lg flex items-center justify-center ${x.c === "destructive" ? "bg-destructive/15 text-destructive" : "bg-primary/15 text-primary"}`}
            >
