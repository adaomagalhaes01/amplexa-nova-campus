...
const d2 = ["Eng","Direito","Gestão","Eco","Cont","Mkt"].map((n, i) => ({ n, a: 60+i*4, b: 50+i*6 }));

function Analytics() {
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold flex items-center gap-2"><BarChart3 className="h-7 w-7 text-primary" /> Analytics</h1>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <Card className="p-5">
          ...
          <div className="h-64">
            <ResponsiveContainer>
              <AreaChart data={d1}>
                <defs><linearGradient id="ar" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="hsl(220 90% 60%)" stopOpacity={0.5}/><stop offset="100%" stopColor="hsl(220 90% 60%)" stopOpacity={0}/></linearGradient></defs>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
                <XAxis dataKey="m" stroke="var(--muted-foreground)" fontSize={11} />
                <YAxis stroke="var(--muted-foreground)" fontSize={11} />
                <Tooltip contentStyle={{background:"var(--card)",border:"1px solid var(--border)",borderRadius:8}} />
                <Area type="monotone" dataKey="v" stroke="hsl(220 90% 60%)" fill="url(#ar)" strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </Card>
          ...
          <div className="h-64">
            <ResponsiveContainer>
              <LineChart data={d1}>
                ...
                <YAxis stroke="var(--muted-foreground)" fontSize={11} />
                <Tooltip contentStyle={{background:"var(--card)",border:"1px solid var(--border)",borderRadius:8}} />
                <Line type="monotone" dataKey="v" stroke="hsl(25 95% 55%)" strokeWidth={3} dot={{r:3}} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </Card>
