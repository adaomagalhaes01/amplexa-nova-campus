...
const kpis = [
  ...
  { l: "Assiduidade", v: "97.2%", d: "+1.4% vs período", i: Activity, c: "from-accent to-accent-glow" },
  { l: "Eventos", v: "48", d: "12 esta semana", i: Calendar, c: "from-primary to-accent" },
  { l: "Estágios activos", v: "1,284", d: "+86 vagas novas", i: Briefcase, c: "from-accent to-primary" },
];

const trend = Array.from({ length: 12 }, (_, i) => ({ m: ["J","F","M","A","M","J","J","A","S","O","N","D"][i], a: 80+Math.sin(i)*8+i, b: 70+Math.cos(i)*6+i }));
const bars = ["Eng. Info", "Direito", "Gestão", "Economia", "Contab.", "Marketing"].map((n, i) => ({ n, v: 60 + (i * 7) % 35 }));
const pie = [{ n: "Aprovados", v: 78 }, { n: "Em risco", v: 14 }, { n: "Reprovados", v: 8 }];
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Aqui está o estado da UÓR — em tempo real.</p>
        </div>
        <Badge className="gradient-primary text-primary-foreground border-0">Sistema · Saudável</Badge>
      </div>
          ...
          <motion.div key={k.l} initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} transition={{delay: i*0.05}}>
            <Card className="p-5 hover:shadow-elegant transition group">
              <div className="flex items-center justify-between mb-3">
                <div className={`h-10 w-10 rounded-xl bg-gradient-to-br ${k.c} flex items-center justify-center`}>
          ...
          <div className="h-64">
            <ResponsiveContainer>
              <AreaChart data={trend}>
                ...
                <Area type="monotone" dataKey="a" stroke="hsl(220 90% 60%)" fill="url(#ga)" strokeWidth={2} />
                <Area type="monotone" dataKey="b" stroke="hsl(25 95% 55%)" fill="url(#gb)" strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </Card>
          ...
          <div className="space-y-2 mt-2">
            {pie.map((p, i) => (
              <div key={p.n} className="flex items-center justify-between text-xs">
                <span className="flex items-center gap-2"><span className="h-2 w-2 rounded-full" style={{background: pieColors[i]}} />{p.n}</span>
          ...
          <div className="space-y-3">
            {activity.map((a, i) => (
              <div key={i} className="flex items-start gap-3 text-sm">
                <div className={`mt-1.5 h-2 w-2 rounded-full shrink-0 ${a.b === "primary" ? "bg-primary" : a.b === "accent" ? "bg-accent" : "bg-destructive"}`} />
