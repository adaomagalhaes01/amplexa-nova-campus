...
function Bio() {
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold flex items-center gap-2"><Fingerprint className="h-7 w-7 text-primary" /> Sistema Biométrico</h1>
        <p className="text-sm text-muted-foreground">Presenças, acessos e monitoramento em tempo real.</p>
      </div>
        ...
        <div className="space-y-2">
          {logs.map((l, i) => (
            <div key={i} className="flex items-center gap-3 py-2 border-b last:border-0 text-sm">
              <div className={`h-8 w-8 rounded-lg flex items-center justify-center shrink-0 ${l.ok? "bg-primary/15 text-primary":"bg-destructive/15 text-destructive"}`}>
